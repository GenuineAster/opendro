...
void draw_keypad(keypad_grid_t *keypad, ui_t *ui, int offset_x, int offset_y) {
	for (int col = 0; col < keypad->num_columns; ++col) {
		for (int row = 0; row < keypad->num_rows; ++row) {
			ui_rect_t rect = {.x = offset_x + (keypad->button_size_x + UI_MARGIN) * col, .y = offset_y + (keypad->button_size_y + UI_MARGIN) * row, .w = keypad->button_size_x, .h = keypad->button_size_y};
			if (ui_button(ui, rect, keypad->buttons[col][row].button_text, 2.f)) {
